...
spawn("gulp.cmd", [], { stdio: 'inherit' }).on('close', function (code) {
  if(code !== 0) {
    process.exit(code);
  }

  exec("git.exe status --porcelain", function (error, stdout, stderr) {
    if (error) {
      console.log(stdout);
      console.log(stderr);
    ...
    inquirer.prompt({
      ...
      spawn("gulp.cmd", ["bump-" + importance.bump], { stdio: 'inherit' }).on("close", function (code2) {
        ...
        spawn("git.exe", ["push"], { stdio: 'inherit' }).on("close", function (code3) {
          if(code3 !== 0) { process.exit(code3); }
          spawn("git.exe", ["push", "--tags"], { stdio: 'inherit' }).on("close", function (code4) {
            if(code4 !== 0) {
              process.exit(code4);
            }
